...
static int read_partition(int fd, int offset, char* data, int size);
// The origin is the file, for example recover from udisk or sdcard
static int read_file(int fd, int offset, char* data, int size);
...
/*
    ...
    error return -1
 */
int check_image(const char* mtddevname, unsigned long image_size)
{
	int size = 32<<9;
	char buffer[16*1024] = "";
	unsigned long crc = 0;
...
/* check product model */
    char model[MAX_MACHINE_MODEL] = {0};
...
int open_partition_path(const char *part_name, int mode, char* path) {
	...
	char mtdname[32];
       ...
       if(p = strstr(v->device,"/dev/block/mtd/by-name/"))
	   strcpy(mtdname,p+23);	
	else
	   strcpy(mtdname,v->device);
...
//================================================================
/*  鍗囩骇/杩樺師绯荤粺鐨勬潵婧愭湁涓ょ:
       鏂囦欢绯荤粺锛屽uDisk:update.img锛孲DCARD:update.img
       鍒嗗尯锛屽BACKUP:update.img
...
//gyq for airis
		LOGI("gyq-erase all user data.\n");
	  erase_volume("/data");
    erase_volume("/cache");
    erase_volume("/mnt/sdcard");
//
	ui->SetProgress(1.0);
    ui->Print("Installation complete.\n");
...
//ps注: Normal use, the user should can't through the other tools to update parameter/kernel/boot/recovery alone
